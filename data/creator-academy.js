(function(){
const artists=[
{name:'Michael Jackson',group:'Dance & Pop',idea:'Listen for a steady dance beat, bass, guitar, claps, and layered singing.',url:'https://music.apple.com/us/search?term=Michael%20Jackson'},
{name:'Travis Scott',group:'Hip-Hop Production',idea:'Listen for deep bass, roomy drums, repeating patterns, sound effects, and changing energy.',url:'https://music.apple.com/us/search?term=Travis%20Scott'},
{name:'Bruno Mars',group:'Pop & Funk',idea:'Listen for bright drums, bass, guitar, horns, and a catchy melody.',url:'https://music.apple.com/us/search?term=Bruno%20Mars'},
{name:'Stevie Wonder',group:'Soul & Keys',idea:'Listen for keyboards, bass, drums, singing, and rich chords.',url:'https://music.apple.com/us/search?term=Stevie%20Wonder'},
{name:'Pharrell Williams',group:'Beat Making',idea:'Listen for simple drum patterns, claps, bass, and space between sounds.',url:'https://music.apple.com/us/search?term=Pharrell%20Williams'},
{name:'Earth, Wind & Fire',group:'Funk & Horns',idea:'Listen for horns, bass guitar, drums, rhythm guitar, and group singing.',url:'https://music.apple.com/us/search?term=Earth%20Wind%20%26%20Fire'},
{name:'Daft Punk',group:'Electronic Music',idea:'Listen for electronic drums, synths, robot-style sounds, and repeating loops.',url:'https://music.apple.com/us/search?term=Daft%20Punk'},
{name:'Hans Zimmer',group:'Movie Music',idea:'Listen for strings, brass, drums, piano, and sounds that build tension.',url:'https://music.apple.com/us/search?term=Hans%20Zimmer'}
];
...
function renderMusic(){document.getElementById('musicDone').textContent=Object.values(completed).filter(Boolean).length;document.getElementById('artistGrid').innerHTML=artists.map((a,i)=>`<div class="artist-card"><div class="music-tag">${a.group}</div><h3>${a.name}</h3><p>${a.idea}</p><a href="${a.url}" target="_blank" rel="noopener"><button class="blue">Open Apple Music</button></a></div>`).join('');document.getElementById('musicGrid').innerHTML=lessons.map((l,i)=>`<div class="card"><div class="music-tag">${l.level}</div><h3>${i+1}. ${l.title}</h3><p>${l.goal}</p><p class="muted">${completed[l.id]?'✅ Finished':'Ready to start'}</p><button onclick="openMusicLesson(${i})">${completed[l.id]?'Do Again':'Start Mission'}</button></div>`).join('')}
let active=0;
window.openMusicLesson=i=>{active=i;const l=lessons[i],a=artists[l.artist];document.querySelectorAll('main > section').forEach(s=>s.classList.add('hidden'));document.getElementById('musicLessonView').classList.remove('hidden');document.getElementById('musicLevel').textContent=l.level;document.getElementById('musicTitle').textContent=l.title;document.getElementById('musicGoal').textContent=l.goal;document.getElementById('musicMission').textContent=l.mission;const listen=document.getElementById('musicListenBtn');listen.textContent='Listen to '+a.name+' in Apple Music';listen.onclick=()=>window.open(a.url,'_blank','noopener');document.getElementById('musicSteps').innerHTML=l.steps.map((x,n)=>`<div class="music-step"><b>Step ${n+1}:</b> ${x}</div>`).join('');document.getElementById('musicBuildInfo').innerHTML=l.gb?`<b>Project Check</b><p><b>Tempo:</b> ${l.gb.tempo}</p><p><b>Tracks:</b> ${l.gb.tracks.join(', ')}</p><ul>${l.gb.check.map(x=>`<li>${x}</li>`).join('')}</ul>`:'<b>Listening Check</b><p>Tell a parent three sounds you heard.</p>'};
